...
use hub::WsPlayerHub;
use messages::{Connect, Disconnect, RodioCommandMessage, WsRodioStateMessage};
...
pub struct WsPlayerClientController {
    id: Uuid,
    hub_addr: Addr<WsPlayerHub>,
    hb: Instant,
}

impl WsPlayerClientController {
    pub fn new(hub_addr: Addr<WsPlayerHub>) -> Self {
        Self {
            id: Uuid::new_v4(),
            hub_addr,
            hb: Instant::now(),
...
impl Actor for WsPlayerClientController {
...
impl StreamHandler<Result<ws::Message, ws::ProtocolError>> for WsPlayerClientController {
    fn handle(
        &mut self,
...
impl Handler<WsRodioStateMessage> for WsPlayerClientController {
    type Result = ();
